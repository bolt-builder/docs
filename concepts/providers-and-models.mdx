...
description: "Bolt supports any LLM provider via the Vercel AI SDK. Authenticate providers, list models, tune reasoning effort, and race models with best-of-N or arena."
---
...
`bolt arena` takes the same idea further: it races several agents on the same task, but each contender gets its own git worktree so they can freely edit files without stepping on each other. When they finish, a judge model ranks them on **the final answer plus the actual git diff** — not just the text they wrote.
