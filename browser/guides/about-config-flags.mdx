...
description: A list of useful flags in Bolt Browser's Advanced Preferences page (about:config).
---
These are a variety of settings that can change Bolt's appearance, behavior, and more. To access them, go to `about:config` in Bolt Browser, and search for the title of the flag. Some of these flags may require a restart to work.

### Appearance
#### `zen.theme.content-element-separation`
Changes the size of the border around the browser window. 8 by default.
#### `zen.theme.accent-color`
The color hex value of Bolt's main accent color.
...
Shows a splash screen briefly when Bolt is opened. Enabled by default.

### Transparency & Blur
#### `zen.theme.acrylic-elements`
Windows-specific. Allows use of Windows' Acrylic protocol for a blur effect in Bolt. Enabled by default on Windows.
